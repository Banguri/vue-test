<template>
  <article>
    <div class="groupWrap m-b-20">
      <Default-Form :Fields="DefaultColumn" ref="defaultForm" :propItems="propItems"/>
    </div>
    <div class="btnBox">
      <b-button
          variant="success"
...
//컬럼설정
import {defaultColumn} from "./settingSheets.js";
...
export default {
  ...
  methods: {
    async loadItem() {
      if (!_.isNull(this.SiteID)) {
        const res = await this.$fetch({
          api: {
            path: `/mng/sitSite/read?id=${this.SiteID}`,
            method: "get",
          }
        });
    ...
    async save() {
      const res = await this.$refs.defaultForm.saveSubmit("/mng/sitSite/save");
