	...
	<li v-if="menu.children" class="has-sub" v-bind:class="{ 'active': subIsActive(menu.path), 'expand': this.stat =='expand', 'd-none': this.stat =='hide' }">
		...
		<ul class="sub-menu" v-bind:class="{ 'd-block': this.stat == 'expand', 'd-none': this.stat == 'collapse' }" v-bind:style="{ marginTop: (pageOptions.pageSidebarMinified) ? - (scrollTop + 40) + 'px' : '' }">
			<template v-for="(submenu,index) in menu.children">
				<sidebar-nav-list v-bind:menu="submenu" v-bind:key="submenu.path+'_'+index" ref="sidebarNavList" v-on:calcFloatSubmenu="handleCalcFloatSubmenu" v-on:collapse-other="handleCollapseOther(submenu)"></sidebar-nav-list>
			</template>
		</ul>
	</li>

	<!-- menu without submenu -->
<!--	<router-link v-else v-bind:to="menu.title" v-bind:class="{ 'd-none': this.stat =='hide' }" active-class="active" tag="li" v-on:click.native="collapseOther()">-->
<!--	</router-link>-->
		<li v-else v-bind:class="{ 'd-none': this.stat =='hide' }" active-class="active" >
			<a href="#" v-on:click="collapseOther()">
				<span v-if="menu.badge" class="badge pull-right">{{ menu.badge }}</span>
				<i v-if="menu.icon" v-bind:class="menu.icon"></i>
				<div v-if="menu.img" class="icon-img"><img v-bind:src="menu.img" alt="" /></div>
				<span>
...
import {pushMenu} from '@@/_core/store/tabMenu';
...
export default {
	...
	methods: {
		...
		async collapseOther(){
			this.$emit('collapse-other', this.menu);
			console.log("menuAdd",this.menu);
			//클릭한 메뉴를 스토어에 저장.
			await pushMenu(this.$store.dispatch, this.menu, this.$store.getters["tabMenu/getActiveMenus"]);
		},
